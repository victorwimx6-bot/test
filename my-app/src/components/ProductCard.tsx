        ...
        {multi && (
          <div className="absolute inset-x-1 top-1/2 -translate-y-1/2 flex justify-between pointer-events-none z-10">
            <button 
              ...
              className="bg-black/40 text-white rounded-full p-1 w-6 h-6 flex items-center justify-center pointer-events-auto hover:bg-black/60 transition"
            >
              ‹
            </button>
            <button 
              onClick={() => setImgIdx(i => Math.min(i + 1, product.images.length - 1))} 
              className="bg-black/40 text-white rounded-full p-1 w-6 h-6 flex items-center justify-center pointer-events-auto hover:bg-black/60 transition"
            >
              ›
            </button>
